/**
 * MailSlurp API
 * MailSlurp is an API for sending and receiving emails and SMS from dynamically allocated email addresses and phone numbers. It's designed for developers and QA teams to test applications, process inbound emails, send templated notifications, attachments, and more.  ## Resources  - [Homepage](https://www.mailslurp.com) - Get an [API KEY](https://app.mailslurp.com/sign-up/) - Generated [SDK Clients](https://docs.mailslurp.com/) - [Examples](https://github.com/mailslurp/examples) repository
 *
 * The version of the OpenAPI document: 6.5.2
 * Contact: contact@mailslurp.dev
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 6.2.1.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * AITransformCreateOptions.h
 *
 * 
 */

#ifndef COM_MAILSLURP_CLIENT_MODEL_AITransformCreateOptions_H_
#define COM_MAILSLURP_CLIENT_MODEL_AITransformCreateOptions_H_


#include "CppRestOpenAPIClient/ModelBase.h"

#include "CppRestOpenAPIClient/model/StructuredOutputSchema.h"
#include <cpprest/details/basic_types.h>
#include <vector>

namespace com {
namespace mailslurp {
namespace client {
namespace model {

class StructuredOutputSchema;

/// <summary>
/// 
/// </summary>
class  AITransformCreateOptions
    : public ModelBase
{
public:
    AITransformCreateOptions();
    virtual ~AITransformCreateOptions();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;

    /////////////////////////////////////////////
    /// AITransformCreateOptions members

    /// <summary>
    /// 
    /// </summary>
    utility::string_t getName() const;
    bool nameIsSet() const;
    void unsetName();

    void setName(const utility::string_t& value);

    /// <summary>
    /// 
    /// </summary>
    std::vector<utility::string_t>& getConditions();
    bool conditionsIsSet() const;
    void unsetConditions();

    void setConditions(const std::vector<utility::string_t>& value);

    /// <summary>
    /// 
    /// </summary>
    std::vector<utility::string_t>& getInstructions();
    bool instructionsIsSet() const;
    void unsetInstructions();

    void setInstructions(const std::vector<utility::string_t>& value);

    /// <summary>
    /// 
    /// </summary>
    std::shared_ptr<StructuredOutputSchema> getOutputSchema() const;
    bool outputSchemaIsSet() const;
    void unsetOutputSchema();

    void setOutputSchema(const std::shared_ptr<StructuredOutputSchema>& value);

    /// <summary>
    /// 
    /// </summary>
    utility::string_t getOutputSchemaId() const;
    bool outputSchemaIdIsSet() const;
    void unsetOutputSchemaId();

    void setOutputSchemaId(const utility::string_t& value);

    /// <summary>
    /// 
    /// </summary>
    utility::string_t getExtractPrompt() const;
    bool extractPromptIsSet() const;
    void unsetExtractPrompt();

    void setExtractPrompt(const utility::string_t& value);


protected:
    utility::string_t m_Name;
    bool m_NameIsSet;
    std::vector<utility::string_t> m_Conditions;
    bool m_ConditionsIsSet;
    std::vector<utility::string_t> m_Instructions;
    bool m_InstructionsIsSet;
    std::shared_ptr<StructuredOutputSchema> m_OutputSchema;
    bool m_OutputSchemaIsSet;
    utility::string_t m_OutputSchemaId;
    bool m_OutputSchemaIdIsSet;
    utility::string_t m_ExtractPrompt;
    bool m_ExtractPromptIsSet;
};


}
}
}
}

#endif /* COM_MAILSLURP_CLIENT_MODEL_AITransformCreateOptions_H_ */
